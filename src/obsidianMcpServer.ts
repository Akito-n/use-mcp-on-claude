...
export function configureServer(server: McpServer): void {
  ...
  const getObsidianFiles = async (
    ...
    const normalizedDirPath =
      dirPathStr === "/" ? "" : dirPathStr.replace(/^\/+|\/+$/g, "");
    ...
    if (!fullPath.startsWith(OBSIDIAN_VAULT_PATH)) {
      throw new Error(`Access denied details:
                       ...
                       - fullPath.startsWith check: ${fullPath.startsWith(
                         OBSIDIAN_VAULT_PATH
                       )}
  ...
  server.resource(
    ...
    async (uri, { dirPath = "" }) => {
      try {
        console.error(
          `DEBUG: List resource called with URI: ${uri.href}, dirPath: ${dirPath}`
        );
        ...
        console.error(
          `DEBUG: Returning result with ${result.files.length} items`
        );
        ...
        console.error(
          `DEBUG ERROR STACK: ${error instanceof Error ? error.stack : ""}`
        );
  ...
  server.tool(
    "obsidian-search",
    {
      path: z
        .string()
        .optional()
        .describe("Path to search in (leave empty for root)"),
      ...
      try {
        ...
        if (query && query.trim() !== "") {
          ...
          result.files = result.files.filter(
            (file: FileInfo) => file.name.indexOf(query) !== -1
          );
        }

        let responseText = `Files in ${
  ...
  server.resource(
    ...
    async (uri, { filePath = "" }) => {
      try {
        console.error(
          `DEBUG: Read resource called with URI: ${uri.href}, filePath: ${filePath}`
        );

        const filePathStr = Array.isArray(filePath)
          ? filePath[0]
          : filePath || "";
  ...
  const readObsidianFile = async (
    ...
    console.error(
      `DEBUG: File read successfully, size: ${content.length} bytes`
    );
  ...
  server.tool(
    ...
    async ({ query, path: searchPath = "" }) => {
      try {
        console.error(
          `DEBUG: Content search started with query: "${query}", path: "${path}"`
        );
        const fileListResult = await getObsidianFiles(searchPath);
        console.error(
          `DEBUG: Found ${fileListResult.files.length} files to search in`
        );
        ...
        const filesToSearch = fileListResult.files.filter((file) => {
          ...
          const ext =
            extIndex !== -1 ? file.name.slice(extIndex).toLowerCase() : "";
        ...
        console.error(
          `DEBUG: Search complete, found ${searchResults.length} matching files`
        );
        ...
        if (searchResults.length === 0) {
          ...
          searchResults.forEach((result) => {
            responseText += `${result.fileInfo.name} (${formatFileSize(
